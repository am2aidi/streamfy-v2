            ...
            <div className="grid grid-cols-1 gap-6">
              ...
              {matches.length ? (
                <ResultSection title="Sports">
                  {matches.map((m) => (
                    <ResultItem key={m.id} href={`/sports/${m.id}`} title={`${m.team1.name} vs ${m.team2.name}`} meta={m.league} />
                  ))}
                </ResultSection>
              ) : null}
              ...
              {shorts.length ? (
                <ResultSection title="Shorts">
                  {shorts.map((s) => (
                    <ResultItem key={s.id} href={`/shorts/${s.id}`} title={s.title} meta={s.category.toUpperCase()} />
                  ))}
                </ResultSection>
              ) : null}
