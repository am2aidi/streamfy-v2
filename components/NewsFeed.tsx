...
export function NewsFeed({
  ...
  const updatedLabel = useMemo(
    () => (items.length ? getTranslation(settings.language, 'updatedHourly') : 'No updates yet'),
    [items.length, settings.language],
  )
                ...
                <div className="flex shrink-0 flex-wrap items-center gap-2">
                  {item.videoUrl ? (
                    ...
                    >
                      ...
                      {getTranslation(settings.language, 'trailer')}
                    </button>
                  ) : null}
                  <button
                    onClick={() => setActive(item)}
                  ...
                  >
                    {getTranslation(settings.language, 'readMore')}
                  </button>
                </div>
              </div>
            </article>
